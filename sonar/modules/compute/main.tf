variable "security_group" {
  description = "The security groups assigned to the sonar server"
}

variable "public_subnet" {
  description = "The public subnet IDs assigned to the sonar server"
}
...
resource "aws_instance" "sonar_server" {
  ...
  user_data              = <<-EOF
    ...
    chmod +x ${path.module}/install_sonar.sh

    # Install Jenkins
    ${file("${path.module}/install_sonar.sh")}
  EOF 
  tags = {
    Name = "sonar_server"
  }
